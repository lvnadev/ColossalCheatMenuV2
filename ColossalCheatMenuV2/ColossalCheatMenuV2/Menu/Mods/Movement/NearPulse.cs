            ...
            if (PluginConfig.NearPulse)
            {
                foreach (VRRig vrrig in GorillaParent.instance.vrrigs)
                {
                    if (!vrrig.isOfflineVRRig && !GorillaTagger.Instance.offlineVRRig.mainSkin.material.name.ToLower().Contains("fected"))
                    {
                        ...
                        {
                            GorillaLocomotion.Player.Instance.bodyCollider.attachedRigidbody.AddExplosionForce(PluginConfig.NearPulseAmmount, vrrig.transform.position, PluginConfig.NearPulseDistance);
                        }
                    }
                }
